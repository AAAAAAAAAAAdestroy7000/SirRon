...
$token = @$_POST["token"];
if ($token == "") {
    header("Location: register.php?error=Google token missing");
    exit();
}
$parts = explode(".", $token);
$payload = "";
...
if ($email == "") {
    header("Location: register.php?error=Unable to get email from Google");
    exit();
}
$serverName = "KORU";
...
header("Location: index.php?success=Account created via Google");
exit();
?>
